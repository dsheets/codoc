...
type t = {
  ...
  unit_by_root : root DocOckTypes.Unit.t Lazy.t RootTable.t;
...
let find_import roots unit name =
  ...
  let open DocOckTypes.Unit.Import in
...
let create doc_index =
  ...
  let fetch root =
    ...
    | lazy unit -> unit
  in
  let resolver = DocOck.build_resolver ~equal ~hash lookup fetch in
  let expander = DocOck.build_expander ~equal ~hash fetch in
  let env =
